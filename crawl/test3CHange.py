...
def spiderpage(url):
    try:
        ...
        # print(pagelinks)
        pagelinks = [x for x in pagelinks if "jztest" in x]
        return pagelinks
    except:
        pagelinks = ['http://']
        print("这个网站有点东西")
    ...
    try:
        ...
        if soup.body is not None:
            ...
            if title != None:
                ...
                return "这网站没有灵性1"
        # except :
        #        if html.status_code == 200:return "404页面"
    except:

        if 'jztest' in url:
            dingTalkSend(url)
            file = open('jztest%s.txt' % time.strftime("%Y.%m.%d"), 'a', encoding='utf8')
            file.write(f'{url}\n')
            print("这网站没有灵性")
            return "不可加载"
    ...
    try:
        for murl in pagelinks:
            ...
            #
                ...
                base = "https://www.jztest.com/"
                # for i in pagelinks:
                try:
                    second_path = re.findall("//(.*?).jztest.com/", murl)
...
class findDeadSource():
    ...
    #     return self.source
    # # 获得二级地址
    # def getThescondLIon(self):
    #     return  self.getThescondLIon()
    # # 获得错误地址的一级地址的列表
    def findDeadlineSour(self,getErrorline,getThescondLIon,getSourceUrl,wrongUrl):

        # 一级地址
        # sourece = ['a', 'b', 'c', 'd']
        # b放扫出来的二级地址
        # B = [['b', 'c'], ['a', 'c', "d"], ['a'], ['c', "b"]]
        # 存在该地址的坐标，假设为"b"为错误的地址，对
        C = [i for i, x in enumerate(getThescondLIon()) if wrongUrl in x]
        # print(C)
        getErrorline.append(wrongUrl)
        # 筛选扫出错误地址的一级地址
        errorSource = []
        for i in C:
            errorSource.append(getSourceUrl[i])
        # print(D)
        # 记录错误的地址，和他的一级地址
        print("%s  -->"%wrongUrl, errorSource)
        return errorSource
...
class Spider():
    def __init__(self, url):
        ...
        self.findDeadSource = findDeadSource()  # 将死链源地址查找加入本类
    ...
    def crawler(self, urlcount):
        ...
        while   x<100 and self.linkQuence.unvisited or x == urlcount:
            ...
            if title == "404页面":
                ...
                file = open('FurlsWR%s.txt' % self.local, 'a', encoding='utf8')
        ...
        if len(error) !=0:
            for errorline in error:
                ...
                # print(D)
                # 记录错误的地址，和他的一级地址
                strmsg = "死链：%s  的所有上级目录" % errorline, wrongSource
                dingTalkSend(strmsg)
