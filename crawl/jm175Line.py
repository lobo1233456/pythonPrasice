...
def url_get(num_retries=5):
    ...
    url = "https://www.jm175.cn"
    #    url = "http://"
    try:
        # 做一个user-agent模拟浏览器发送请求,也可以加入其它字段
        kv = {
            'HOST':"https://www.jm175.cn",
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.90 Safari/537.36'}
...
def spiderpage(url):
    try:
        ...
        # print(pagelinks)
        pagelinks = [x for x in pagelinks if "jm175" in x]
        return pagelinks
    except:
        pagelinks = ['http://']
        print("这个网站有点东西")
...
def getTitle(url):
    ...
    try:
        ...
        if 'jm175' in url:
            dingTalkSend(url)
            file = open('jm175%s.txt' % time.strftime("%Y.%m.%d"), 'a', encoding='utf8')
            file.write(f'{url}\n')
            print("这网站没有灵性")
            return "不可加载"
    ...
    try:
        for murl in pagelinks:
            ...
            #
                ...
                base = "https://www.jm175.cn/"
                # for i in pagelinks:
                try:
                    second_path = re.findall("//(.*?).jm175.com/", murl)
    ...
    def crawler(self, urlcount):
        ...
        while self.linkQuence.unvisited or x == urlcount:
            ...
            if title == "404页面":
                ...
                file = open('jm175WR%s.txt' % self.local, 'a', encoding='utf8')
        ...
        if len(error) != 0:
            for errorline in error:
                ...
                # print(D)
                # 记录错误的地址，和他的一级地址
                strmsg = "jm175死链：%s  的所有上级目录" % errorline, wrongSource
                dingTalkSend(strmsg)
